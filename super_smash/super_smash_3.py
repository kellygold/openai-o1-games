...
# Game variables
GRAVITY = 0.5
MAX_JUMP_HEIGHT = 15  # Increased jump height
PLAYER_SPEED = 5
    ...
    def prevent_falling(self):
        ...
        for platform in self.platforms:
            if (self.opponent.rect.bottom == platform.rect.top and
                ...
                on_platform = True
                # Edge detection
                if self.opponent.move_right and self.opponent.rect.right >= platform.rect.right:
                    self.opponent.move_right = False
                    self.direction = 'left'
                if self.opponent.move_left and self.opponent.rect.left <= platform.rect.left:
                    self.opponent.move_left = False
                    self.direction = 'right'
                break

        if not on_platform and self.opponent.on_ground:
